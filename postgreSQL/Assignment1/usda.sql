...
SELECT COUNT(*)FROM weight WHERE gm_wgt=190;
...
SELECT COUNT(*) FROM data_src WHERE year=1990 AND journal='Cereal Foods World';
...
Q12. How many records in food_des are not about food group Breakfast Cereals.  The field fdgrp_cd is an id field and you will have to find it in fd_group for fddrp_desc = ' Breakfast Cereals'.
SELECT COUNT(*) FROM fd_group WHERE fddrp_desc='Breakfast Cereals';

Q13. Find all the records in data sources that where between 1990 and 2000 and either 'J. Food Protection' or 'Food Chemistry'.
SELECT COUNT (*) FROM data_src WHERE year BETWEEN 1990 AND 2000;
SELECT COUNT (*) FROM data_src WHERE journal IN ('J.Food Protection','Food Chemistry');
...
SELECT COUNT(*) FROM data_src WHERE year IN (1960,1970,1980,1990,2000);